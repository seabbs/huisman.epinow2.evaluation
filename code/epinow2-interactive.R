...
# observations and noise used when sampling from delays).
# This causes EpiNow2 issues as it is designed to fit to real world data where
# observation error is likely to be present.
simulated_cases <- simulated_cases[, confirm := map_dbl(confirm, ~ rpois(1, .))]
...
approximate_lognormal <- function(param_def) {
  estimate_delay(rgamma(1000, shape =  param_def$shape, scale = param_def$scale),
                 bootstraps = 1, bootstrap_samples = 1000, max_value = 15)
}
...
# observation (introduced above to add stochasticity to the Huisman et al simulation). 
# here we reduce the control parameters used for stan as the deterministic data is difficult for the
# stochastic algorithm to fit to.
estimate_with_epinow2 <- function(...) {
  estimate_infections(reported_cases = simulated_cases, 
                      generation_time = generation_time,
                      delays = delay_opts(incubation_period, reporting_delay),
                      stan = stan_opts(control = list(adapt_delta = 0.9, max_treedepth = 10)),
...
epinow2_Re_generative <- estimate_with_epinow2(rt = rt_opts(prior = list(mean = 3, sd = 0.5)))
